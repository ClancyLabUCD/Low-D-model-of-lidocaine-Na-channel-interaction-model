//
//  Moreno_Drug_Dynamics.h
//  ten_Tusscher_single_cell_with_Moreno_2011_and_Simple_Lidocaine_model
//
//  Created by Steffen Docken on 8/4/19.
//  Copyright © 2019 Steffen Docken (Lewis Lab). All rights reserved.
...
#ifndef Moreno_Drug_Dynamics_h
#define Moreno_Drug_Dynamics_h
...
void Moreno_Drug_Dynamics(){
    Cell_param Cell;
    int S1_cycle;
    
    double t, tTemp, interval;
    
    double neut_Drug_I = 0.0;
    double neut_Drug_CO = 0.0;
    double charge_Drug = 0.0;
    //These variables will hold the fraction of channels with neutral drug bound to inactivated channels, neutral drug bound to closed or open channels, and charged drug bound to closed or open channels, repsectively
    ...
    for (int Drug_conc_counter = 0; Drug_conc_counter < 3; Drug_conc_counter++) { //Loops through the various drug concentrations
        ...
        }//recording the drug concentration value for the file name
        ...
        result_file.open((Na_model_str+ "_Moreno_Drug_Dynamics_dt" + dt_val + "_Lido_conc_" + Drug_conc + "_konfac" + k_on_fac_str + "_kofffac" + k_off_fac_str + "_stimdur1.txt").c_str());            // This file contains all of the voltages for each cell (the master-esque file) for the final 5 beats
        result_file0.open((Na_model_str+ "_Moreno_Drug_Dynamics_dt" + dt_val + "_Lido_conc_" + Drug_conc + "_konfac" + k_on_fac_str + "_kofffac" + k_off_fac_str + "_stimdur1_param.txt").c_str()   );    // These files hold data for each cell cycle (to compare V_min, V_max, V_90 etc. to compare between cells)
        result_file3.open((Na_model_str+ "_Moreno_Drug_Dynamics_dt" + dt_val + "_Lido_conc_" + Drug_conc + "_konfac" + k_on_fac_str + "_kofffac" + k_off_fac_str + "_stimdur1_IC_check.txt").c_str()   );    // This file contains all the cell variables at two points to check if steady state was reached.
        result_file4.open((Na_model_str+ "_Moreno_Drug_Dynamics_dt" + dt_val + "_Lido_conc_" + Drug_conc + "_konfac" + k_on_fac_str + "_kofffac" + k_off_fac_str + "_stimdur1_initial_APs.txt").c_str()   );    // This file contains all the variables in the result_file, but for the first 50 APs
        ...
        for (S1_cycle = 0; S1_cycle <=500; S1_cycle = S1_cycle + 1){                    //Enter the number of beats that you want to simulate
            ...
            else {interval=750.0;} //This is a frequency of 80 BPM                                                    //Enter the BCL of the simulation here
            ...
            for (tTemp = 0; tTemp <= interval; tTemp=tTemp+dt){
                ...
                if (counter%10==0 && S1_cycle >=495 ){
                    neut_Drug_I = Cell.D_IC3 + Cell.D_IC2 + Cell.D_IF + Cell.D_IM1 + Cell.D_IM2 + Cell.D_OS;
                    neut_Drug_CO = Cell.D_C3 + Cell.D_C2 + Cell.D_O + Cell.D_C1;
                    charge_Drug = Cell.DIC3 + Cell.DIC2 + Cell.DIF + Cell.DIM1 + Cell.DIM2 + Cell.DC3 + Cell.DC2 + Cell.DO + Cell.DOS + Cell.DC1;
                    
                    result_file << t << ", " << Cell.V << ", " << Cell.I_Na<< ", " << Cell.I_Ca_L << ", " << Cell.I_Na_L << ", "<< Cell.I_up << ", "<< Cell.I_leak << ", "  << Cell.I_Ks << ", " << Cell.Ca_in << ", " << Cell.Ca_ss << ", " << neut_Drug_I << ", " << neut_Drug_CO << ", " << charge_Drug << std::endl;}
